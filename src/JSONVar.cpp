...
int JSONVar::stringLength() const {
  if (!cJSON_IsString(_json)) {
    return -1;
  }
  return strlen(_json->valuestring);
}

int JSONVar::arrayLength() const {
  if (!cJSON_IsArray(_json)) {
    return -1;
  }
...
int JSONVar::objectLength() const {
  if (!cJSON_IsObject(_json)) {
    return -1;
  }
  return cJSON_GetArraySize(_json);
...
int JSONVar::length() const {
  if (cJSON_IsString(_json)) {
    return strlen(_json->valuestring);
  } else if (cJSON_IsArray(_json) || cJSON_IsObject(_json)) {
    return cJSON_GetArraySize(_json);
  } else {
    return -1;
  }
...
bool JSONVar::isNull() const {
  return _json == NULL || cJSON_IsInvalid(_json) || cJSON_IsNull(_json);
}
bool JSONVar::isBool() const {
  return cJSON_IsBool(_json);
}
bool JSONVar::isNumber() const {
  return cJSON_IsNumber(_json);
}
bool JSONVar::isString() const {
  return cJSON_IsString(_json);
}
bool JSONVar::isArray() const {
  return cJSON_IsArray(_json);
}
bool JSONVar::isObject() const {
  return cJSON_IsObject(_json);
}
